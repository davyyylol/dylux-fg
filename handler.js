        ...
        case 'remove':
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    let pp = 'https://i.imgur.com/JeZfebi.jpg'
                    let ppgp = 'https://i.imgur.com/JeZfebi.jpg'
                    try {
                        pp = await this.profilePictureUrl(user, 'image')
                        ppgp = await this.profilePictureUrl(id, 'image')
                        } finally {
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Benvenuto @user').replace('@group', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'Desconocido') :
                            ...
                            let wel = API('fgmods', '/api/welcome', {
                                ...
                                profile: pp,
                                background: 'https://i.imgur.com/MCMFiZJ.jpg'
                            }, 'apikey')
                            ...
                            let lea = API('fgmods', '/api/goodbye', {
                                ...
                                membercount: groupMetadata.participants.length,
                                profile: pp,
                                background: 'https://i.imgur.com/zyc5hCl.jpg'
                            }, 'apikey')
                            // this.sendFile(id, pp, 'pp.jpg', text, null, false, { mentions: [user] })
                            this.sendButton(id, text, fgig, action === 'add' ? wel : lea, [     
                             [(action == 'add' ? '' : ''), (action == 'add' ? '' : '')] ], null, {mentions: [user]})
...
global.dfail = (type, m, conn) => {
    ...
    if (msg) return conn.sendButton(m.chat, msg, fgig, null, [['⦙☰ Menu', '/menu'] ], m)
}
